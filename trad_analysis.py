...
class Analysis:
    ...
    def get_test(self, money = 1000000,strategy_trad = 'position', signal= 'two_ma_samp',
                 signal_param = {'ma_samp_slow_param' : 350,
                          'ma_samp_fast_param' : 100,
                          'pct_change' : 0.01}):
        # Разметка данных по выбранному типу сигналов
        if signal == 'two_ma_samp': df_dikt = self._get_resempl(self._get_two_ma_samp(signal_param))
        else: df_dikt = self._get_resempl(self._get_two_ma_samp()) # На время пока нет других стратегий
        # Получение размеченного df с сигналами для сделок
        self.df_cignal_dict = df_dikt
    ...
    def _get_two_ma_samp(self, param):
